...
btn[1].addEventListener('click', function () {
  ...
  //corrige valor on
  var buscar = "on";
  var indiceTag = arrTags.indexOf(buscar);
  var indiceBad = arrBads.indexOf(buscar);
  if(indiceTag >= 0 || indiceBad >= 0){
    arrTags.splice(indiceTag, 1);
    indiceTag = arrTags.indexOf(buscar);
    arrBads.splice(indiceBad, 1);
    indiceBad = arrBads.indexOf(buscar);
  }

  //Manda valores para input
  document.querySelector('#valor').value = arrTags.join('<br>')
  document.querySelector('#bad').value = arrBads.join('<br>')
  ...
  exbData.innerHTML = `<b>Date:</b> ${data.getDate() < 10 ? '0' + data.getDate() : data.getDate()}/${data.getMonth() + 1 < 10 ? '0' + (data.getMonth() + 1) : data.getMonth() + 1}/${data.getFullYear()}`;
...
dragElement(document.getElementById("gerar"));
